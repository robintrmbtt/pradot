...
class ReadImage:
    ...
    def __call__(self, path):
        if not self.is_gt:
            ...
            x = torchvision.io.read_image(path, mode=torchvision.io.ImageReadMode.GRAY)
...
class CustomDataset(Dataset):
    ...
    def prepare_data(self, data):
        def prep_gt(gt):
            if gt is not None:
                return [self.io_transforms_gt(gt_i) for gt_i in gt]
            return gt
...
def create_dataset_mvtec_loco(data_path, obj, img_size, n_train=None, n_test=None, stages=['train', 'validation', 'test']):
    data_train = np.array([
    ...
    data_val = np.array([
        {'image': os.path.join(data_path, obj, 'validation', 'good', filename), 
        'gt': None,
        }
        for filename in sorted(os.listdir(os.path.join(data_path, obj, 'validation', 'good')))
    ])

    if n_train is not None:
        data_train = data_train[:n_train]
        data_val = data_val[:n_train]

    data_test = []
    data_test += [(
        {'image': os.path.join(data_path, obj, 'test', 'logical_anomalies', filename+'.png'), 
        'gt': [
            os.path.join(data_path, obj, 'ground_truth', 'logical_anomalies', filename, gt_id)
            for gt_id in sorted(os.listdir(os.path.join(data_path, obj, 'ground_truth', 'logical_anomalies', filename)))
            ],
        'infos': {
            'img_id': filename,
            'gt_id': [gt_id.split('.')[0] for gt_id in sorted(os.listdir(os.path.join(data_path, obj, 'ground_truth', 'logical_anomalies', filename)))],
            'type': 'logical_anomalies'
            }
        }
    ) for filename in sorted(os.listdir(os.path.join(data_path, obj, 'ground_truth', 'logical_anomalies')))]

    data_test += [(
        {'image': os.path.join(data_path, obj, 'test', 'structural_anomalies', filename+'.png'), 
        'gt': [
            os.path.join(data_path, obj, 'ground_truth', 'structural_anomalies', filename, gt_id)
            for gt_id in sorted(os.listdir(os.path.join(data_path, obj, 'ground_truth', 'structural_anomalies', filename)))
            ],
        'infos': {
            'img_id': filename,
            'gt_id': [gt_id.split('.')[0] for gt_id in sorted(os.listdir(os.path.join(data_path, obj, 'ground_truth', 'structural_anomalies', filename)))],
            'type': 'structural_anomalies'
            }
        }
    ) for filename in sorted(os.listdir(os.path.join(data_path, obj, 'ground_truth', 'structural_anomalies')))]

    data_test += [(
        {'image': os.path.join(data_path, obj, 'test', 'good', filename), 
        'gt': None,
        'infos': {
            'img_id': filename,
            'type': 'good'
    ...
    ) for filename in sorted(os.listdir(os.path.join(data_path, obj, 'test', 'good')))]
...
class MVTecLocoDataModule(LightningDataModule):
    def __init__(self, data_path, obj, img_size, batch_size, n_train=None, n_test=None, stages=['train','val','test'], **kwargs):
        super().__init__()
    ...
    def setup(self, stage=None):
        self.data = create_dataset_mvtec_loco(
...
## Evaluation callbacks for MVTec AD LOCO
class EvalMVTecADLoco(Callback):
    ...
    def on_test_start(self, trainer, pl_module):
        ...
        self.gt_and_pred = {
            # 'gts_img': [],
            # 'gts_pixel': [],
            'preds_img_dist': [[] for _ in range(len(pl_module.model.out_indices)+int(len(pl_module.model.out_indices)>1))],
            'preds_img': [[] for _ in range(len(pl_module.model.out_indices)+int(len(pl_module.model.out_indices)>1))],
            'preds_pixel': [[] for _ in range(len(pl_module.model.out_indices)+int(len(pl_module.model.out_indices)>1))],
        }

        # create a copy of the dataset for evaluation
        for defect in ['good', 'logical_anomalies', 'structural_anomalies']:
            if defect != 'good':
        ...
        shutil.copyfile(os.path.join(trainer.datamodule.data_path, trainer.datamodule.obj, 'defects_config.json'),
                        os.path.join(self.save_dir, 'copy_dataset', trainer.datamodule.obj, 'defects_config.json'))
    ...
    def on_test_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx = 0):
        self.results[batch_idx] = {
            'image': batch['image'][0].float().cpu().detach().numpy(),
            'gt': torch.max(torch.concat(batch['gt']), dim=0).values[0].float().cpu().numpy() if 'gt' in batch else np.zeros(pl_module.model.input_size),
            'infos': batch['infos'],
        ...
        if batch['infos']['type'][0] != 'good':
            os.makedirs(os.path.join(
                self.save_dir, 'copy_dataset', trainer.datamodule.obj, 'ground_truth', batch['infos']['type'][0], batch['infos']['img_id'][0]
                ), exist_ok=True)
            for i in range(len(batch['infos']['gt_id'][0])):
                gt = batch['gt'][0][i][0].cpu().numpy()
                gt = np.clip(gt, 0, 1)
                ...
                gt_img.save(os.path.join(
                    self.save_dir, 'copy_dataset', trainer.datamodule.obj, 'ground_truth',
                        batch['infos']['type'][0], batch['infos']['img_id'][0], f"{batch['infos']['gt_id'][0][i]}.png"
                    ))
    ...
    def on_test_end(self, trainer, pl_module):
        ...
        for idx, res in self.results.items():
            for k in range(len(res['anomap'])):
                ...
                for ax in axs:
                    ax.axis('off')
                plt.suptitle(f'Anomaly map for image {idx} scale {pl_module.model.out_indices[k]}')
                axs[0].imshow(np.transpose(res['image'],(1,2,0)))
                axs[0].imshow(res['anomap'][k], alpha=0.7)
                axs[1].imshow(res['gt'])
                plt.savefig(
                    os.path.join(self.save_dir, 'predictions', f'{idx}_scale_{pl_module.model.out_indices[k]}.png'), dpi=400, bbox_inches='tight'
                    )
                plt.close(fig)
            ...
            if len(res['anomap']) > 1:
                ...
                for ax in axs:
                    ax.axis('off')
                plt.suptitle('Anomaly map for image {batch_idx} aggregated')
                axs[0].imshow(np.transpose(res['image'],(1,2,0)))
                axs[0].imshow(anomap, alpha=0.7)
                axs[1].imshow(res['gt'])
                plt.savefig(os.path.join(self.save_dir, 'predictions', f'{idx}_aggregated.png'), dpi=400, bbox_inches='tight')
                plt.close(fig)
        ...
        os.system(f"python {os.path.join(trainer.datamodule.data_path, 'mvtec_loco_ad_evaluation', 'evaluate_experiment.py')} \
            --object_name {trainer.datamodule.obj} \
        ...
        if trainer.logger is not None:
            with open(os.path.join(self.save_dir, 'metrics.json'), 'r') as f:
                ...
                trainer.logger.log_metrics({
                    'classif/auroc': metrics['classification']['auc_roc']['mean'],
                    'classif/auroc/logical_anomalies': metrics['classification']['auc_roc']['logical_anomalies'],
                    'classif/auroc/structural_anomalies': metrics['classification']['auc_roc']['structural_anomalies'],
                    'seg/auspro5': metrics['localization']['auc_spro']['mean']['0.05'],
                    'seg/auspro5/logical_anomalies': metrics['localization']['auc_spro']['logical_anomalies']['0.05'],
                    'seg/auspro5/structural_anomalies': metrics['localization']['auc_spro']['structural_anomalies']['0.05'],
                })
